...
/* ============================================================
 * v4.5.0（2026-09-11，UA 同步修复：GB 会话绑定完整浏览器 UA）
 * - 新增：打开面板/发送 mod 时自动 POST /api/settings { gbUserAgent } 同步当前浏览器 UA
 *   （GB 会话绑定登录时的完整 UA：OS+浏览器+版本号全部精确匹配，不匹配则 _bIsLoggedIn=false）
 * - 效果：从登录 GB 的浏览器打开油猴面板即可自动修复 UA 不匹配问题
...
(function () {
    ...
    // 「填写和读取分离…添加后的服务端用下拉列表展示选择，不能被修改只能删除」
    ...
    function loadServerList() {
        ...
        // 【原代码】只记一条 SRV_KEY。【改为】「下拉列表」【思路】旧地址迁进清单
    ...
    async function syncUserAgent(base, session) {
        try {
            const headers = Object.assign({ "Content-Type": "application/json" }, session ? { "Cookie": "session=" + session } : {});
